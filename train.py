...
for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        loss_char0 = criterion_char(restored[0], target) + criterion_ssim(restored[0], target)
        loss_char1 = criterion_char(restored[1], target) + criterion_ssim(restored[1], target)
        loss_char2 = criterion_char(restored[2], target) + criterion_ssim(restored[2], target)
        loss_char3 = criterion_char(restored[3], target) + criterion_ssim(restored[3], target)
        loss = loss_char0+0.1*loss_char1+0.1*loss_char2+0.1*loss_char3
    ...
    if epoch%opt.TRAINING.VAL_AFTER_EVERY == 0:
        ...
        for ii, data_val in enumerate((val_loader), 0):
            ...
            input_ = data_val[1]
            [Iorg, row, col, Ipad, row_new, col_new] = imread_CS_py(input_, 64)

            with torch.no_grad():
                restored = model_restoration(Ipad.cuda())

            restored = restored[0].permute(0, 2, 3, 1).cpu().detach().numpy().squeeze(0)
            restored = restored[:row, :col, :]
